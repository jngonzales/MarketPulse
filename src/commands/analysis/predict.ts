...
export async function execute(interaction: ChatInputCommandInteraction) {
  ...
  try {
    ...
    const marketDataService: MarketDataService = client.marketData;
    const chartService: ChartService = client.chart;
    ...
    // Prefer database historical data if available
    const daysMap: Record<string, number> = { '1d': 1, '1w': 7, '1m': 30 };
    const days = daysMap[timeframe] ?? 30;
    const historicalRows = await marketDataService.getHistoricalData(coin, days);
    const historicalData = historicalRows.length
      ? historicalRows.map((r: any) => r.price)
      : await getHistoricalPriceData(coin);

    // Calculate technical indicators
    const technicalAnalysis = calculateTechnicalIndicators(historicalData);

    // Generate predictions
    const predictions = generatePredictions(currentData.current_price ?? currentData.price, technicalAnalysis, historicalData);
